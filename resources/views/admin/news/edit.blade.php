...
<div class="space-y-6">
    <div class="rounded-lg bg-white p-6 shadow-sm">
        <h1 class="text-2xl font-bold text-slate-900 mb-6">Modifier l'Actualité</h1>
        ...
        <form action="{{ route('admin.news.update', $news) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required
                          class="w-full rounded-lg border border-slate-300 px-4 py-2 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20">{{ old('content', $news->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="is_published" class="text-sm font-medium text-slate-700">Publié</label>
            </div>
