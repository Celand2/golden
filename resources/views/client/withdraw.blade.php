...
<div class="space-y-4 px-3 pb-24 pt-4">
    ...
    <div class="mx-auto max-w-xl rounded-3xl bg-white p-8 shadow-lg">
        ...
        $isWithinHours = $now->hour >= 7 && $now->hour < 21;
            ...
            <div class="mb-6 rounded-2xl bg-red-50 p-4 border border-red-200">
                <p class="text-sm font-semibold text-red-900">🚫 Retrait non autorisé</p>
                <p class="text-xs text-red-700 mt-1">Vous devez avoir un investissement VIP actif et avoir réclamé au moins un gain journalier avant de pouvoir effectuer un retrait.</p>
            </div>
            @endif
            ...
            <form method="POST" action="{{ route('client.withdraw') }}" class="space-y-5" id="withdrawForm" onsubmit="return validateWithdrawalForm(event)">
                ...
                <button
                   ...
                   {{ !$isWithinHours || $hasPendingWithdrawal || !$hasDailyProfit ? 'disabled' : '' }}
    ...
    <script>
        ...
        const isWithinHours = {
            {
                $isWithinHours ? 'true' : 'false'
            }
        };
        const hasPendingWithdrawal = {
            {
                $hasPendingWithdrawal ? 'true' : 'false'
            }
        };
        ...
        function calculateFees() {
            ...
            if (amount >= 5000 && amount <= maxBalance) {
                ...
                feesSection.classList.add('hidden');
                submitBtn.disabled = !isWithinHours || hasPendingWithdrawal || !hasDailyProfit;
            }
